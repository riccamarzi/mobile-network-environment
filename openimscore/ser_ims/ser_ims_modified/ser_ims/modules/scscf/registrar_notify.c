...
static str unknown_param_s_body={"\"%.*s\"", 1};
static str unknown_param_s_body_escaped={"\"&lt;%.*s&gt;\"", 1};
						...
						while(c){
							...
							for(cp=c->parameters;cp;cp=cp->next){
								...
								if(cp->value.len == 0) {
									...
									if(cp->value.s[0] == '<' && cp->value.s[cp->value.len - 1] == '>') {
										sprintf(pad.s, unknown_param_s_body_escaped.s, cp->value.len-2, cp->value.s + 1);
										pad.len = strlen(pad.s);
										STR_APPEND(buf, pad);
									
									} else {
										sprintf(pad.s, unknown_param_s_body.s, cp->value.len, cp->value.s);
										pad.len = strlen(pad.s);
										STR_APPEND(buf, pad);
									}
